...
function Controls({ 
  ...
  return (
    <View className="flex-row justify-center items-center py-5 px-5 bg-pink-50/90 gap-8">
      <TouchableOpacity 
        className={`w-14 h-14 rounded-full justify-center items-center shadow-lg ${
          isMicEnabled ? 'bg-pink-500' : 'bg-red-500'
        }`}
      ...
      <TouchableOpacity 
        className={`w-14 h-14 rounded-full justify-center items-center shadow-lg ${
          isCameraEnabled ? 'bg-pink-500' : 'bg-red-500'
        }`}
...
export default function CallScreen() {
  const { room: roomName, mic, cam, avatarName, language } = useLocalSearchParams<{ 
    room: string; 
    mic: string; 
    cam: string; 
    avatarName: string; 
  ...
  // Debug logging
  console.log("CallScreen params:", { roomName, mic, cam, avatarName, language, displayName });
    ...
    (async () => {
      try {
        ...
        if (json.avatar_invited) {
          ...
          setAvatarStatus(`Avatar ${avatarName} invitation failed or not configured`);
        }
      } catch (e: any) {
  ...
  }, [roomName, displayName, mic, cam, avatarName, language, deviceId]);
